...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	Если ЭтоУправлениеСеансами Тогда
		...
		Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды,
			...
			|Составляющая фильтра задается в формате 'appid=приложение1;приложение2|name=username1;username2|spid=24234;234223'.
			|Предусмотрено три фильтра - по номеру сеанса (spid), по имени приложения (appid) и по имени пользователя 1С (name).
			...
			|Пример: ... kill -filter appid=Designer|name=регламент;администратор|spid=283921;32983219 ...");
...
Процедура УдалитьВсеСеансыИСоединенияБазы()
	...
	Для НомерПопытки = 1 По Настройки.ЧислоПопыток Цикл
		Попытка
		...
		Исключение
			...
			Лог.Информация("Попытка удаления сеансов №" + НомерПопытки + " неудачна. Текст ошибки:
			|" + ОшибкаУдаленияСеансов);						
		КонецПопытки;
	КонецЦикла;
...
Процедура УстановитьСтатусБлокировкиСеансов(Знач Блокировать)
	...
	ВремяОкончанияБлокировки = Настройки.ВремяОкончанияБлокировки;

	КомандаВыполнения = МенеджерRac.СтрокаЗапускаКлиента() + СтрШаблон("infobase update --infobase=""%3""%4 --cluster=""%1""%2 --sessions-deny=%5 --denied-message=""%6"" --denied-from=""%8"" --denied-to=""%9"" --permission-code=""%7""",
		...
		ВремяБлокировки,
		ВремяОкончанияБлокировки) + " " + Настройки.АдресСервераАдминистрирования;
	...
	Для НомерПопытки = 1 По Настройки.ЧислоПопыток Цикл
		...
		Исключение
			...
			Лог.Информация("Попытка запуска rac №" + НомерПопытки + " неудачна. Текст ошибки:
			|" + ОшибкаЗапускаRac);			
		КонецПопытки;
	КонецЦикла;
...
Функция ПолучитьСписокСеансов()
	...
	Для Каждого ТекПроцесс Из Соединения Цикл
		...
		Если Не МенеджерФильтраПриложений.СеансПодходит(ТекПроцесс["session-id"], ТекПроцесс["app-id"], ТекПроцесс["user-name"]) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОтключитьСеанс(Знач Сеанс)
	...
	МенеджерRac.ИдентификаторКластера(ОписаниеКластера),
	МенеджерRac.КлючиАвторизацииВКластере(), Сеанс.Идентификатор) + " " + Настройки.АдресСервераАдминистрирования;
...
Функция ПолучитьСоединенияРабочегоПроцесса(Знач РабочийПроцесс)
	...
	Для Каждого ТекПроцесс Из Соединения Цикл
		Если ВРег(ТекПроцесс["app-id"]) = "RAS"
			Или Не МенеджерФильтраПриложений.СеансПодходит(ТекПроцесс["session-id"], ТекПроцесс["app-id"], ТекПроцесс["user-name"]) Тогда
			Продолжить;
		КонецЕсли;
