...
Процедура ПодключитьсяКХранилищу(Знач ПутьХранилища, Знач Логин, Знач Пароль,
								...
								Знач ЗаменитьКонфигурациюБД = Истина, Расширение = "") Экспорт
	...
	Попытка
		ХранилищеКонфигурации.ПодключитьсяКХранилищу(ИгнорироватьНаличиеПодключеннойБД, ЗаменитьКонфигурациюБД, Расширение);
...
//
Процедура ОтключитьсяОтХранилища(Знач ДополнительныеКлючиЗапуска = "", Расширение = "") Экспорт
	...
	Если Не ПустаяСтрока(Расширение) Тогда//11111
...
Процедура СохранитьВерсиюХранилищаРасширенияВФайл(Знач ПутьХранилища, Знач ЛогинАдминистратора, Знач ПарольАдминистратора,
				Знач ВерсияХранилища, Знач ПутьКФайлу, Знач Расширение) Экспорт

		Лог.Информация("Выполняю выгрузку версии конфигурации из хранилища");

		ТекущаяПроцедура = "ВыгрузитьВерсиюХранилищаРасширенияВФайл";

		Ожидаем.Что(ПутьХранилища, ТекущаяПроцедура + " не задана строка подключения к хранилищу расширения").Заполнено();
		Ожидаем.Что(ЛогинАдминистратора, ТекущаяПроцедура + " не задан пользователь хранилища расширения").Заполнено();

		КонтекстБД = УправлениеКонфигуратором.ПолучитьКонтекст();
		Если Не ЗначениеЗаполнено(КонтекстБД.КлючСоединенияСБазой) Тогда
			Лог.Предупреждение("При сохранении версии хранилища расширения в файл не определена БД, создаем на основе временной БД");
			УправлениеКонфигуратором.СоздатьФайловуюБазу(КаталогВременнойИБ);
			УправлениеКонфигуратором.УстановитьКонтекст("/F " + КаталогВременнойИБ, "", "");
		КонецЕсли;
		...
		Попытка
			ХранилищеКонфигурации.СохранитьВерсиюКонфигурацииРасширенияВФайл(ВерсияХранилища, ПутьКФайлу, Расширение);
