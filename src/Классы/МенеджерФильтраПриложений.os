...
Функция СеансПодходит(НомерСеанса, Приложение, Пользователь) Экспорт
	...
	Если ФильтрСеансов.Свойство(strSpid) Тогда
		СеансВключенВФильтр = ПараметрСеансаВФильтре(strSpid, НомерСеанса);
	ИначеЕсли ФильтрСеансов.Свойство(strAppid) И ФильтрСеансов.Свойство(strName) Тогда
		СеансВключенВФильтр = ПараметрСеансаВФильтре(strAppid, Приложение);
		
		Если СеансВключенВФильтр Тогда
			СеансВключенВФильтр = ПараметрСеансаВФильтре(strName, Пользователь);
		КонецЕсли;
	Иначе
		Если ФильтрСеансов.Свойство(strAppid) Тогда
			СеансВключенВФильтр = ПараметрСеансаВФильтре(strAppid, Приложение);	
		КонецЕсли;

		Если ФильтрСеансов.Свойство(strName) Тогда
			СеансВключенВФильтр = ПараметрСеансаВФильтре(strName, Пользователь);	
		КонецЕсли;
	КонецЕсли;
